...
def write_auto_js(records: list[dict]):
    # Keep rejected records in the JSON audit data so discovery will not add
    # them again, but never expose them as browseable directory listings.
    public_records = [record for record in records if record.get("reviewStatus") != "rejected"]
