#!/usr/bin/env python3
"""Discover, validate, categorize, and stage new AI tools for review.
...
def categorize(text: str, title: str = ""):
    ...
    # Automated qualification requires the product title itself to communicate
    # category fit. This prevents technical implementation details in a Show HN post from
...
def candidate_from_hn(hit: dict, source_name: str):
    ...
    # capability. This avoids staging unrelated projects that only mention AI
    # incidentally in their implementation notes.
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Show candidates without staging them")
    ...
    minimum = float(discovery_config.get("minimumConfidence", 0.55))
    limit = int(discovery_config.get("maxNewToolsPerRun", 8))
    public_statuses = set(discovery_config.get("publicationStatuses", ["editorially-corrected", "directory-only"]))
    accepted = []
    ...
    if accepted:
        ...
        for item in accepted:
            log("tool_staged_for_review", {"slug": item["slug"], "category": item["category"], "source": item["sourceName"]})
    write_auto_js(existing, public_statuses)
